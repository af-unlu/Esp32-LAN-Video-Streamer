...
extern TaskHandle_t ntp_task_t;
...
void DataProcess(void){
    ...
    while(1)
    {
        ...
        printf("\n%s",log_text);
        FILE *file = fopen("/root/log.csv", "a");
        if (file != NULL)
        {
...
void VideoStream_UC(void){
    ...
    while (1) {
        ...
        while (1) {
            ...
            //video arkasına telemetri paketi koymak ??
            //ornehin 90 byte boşluk var gidip ben 50 byte veri yazabilirim
            //38 tane A sonra 50 byte paket sonra 2 byte remainderl,remainderh
            /*if(_remainderTemp <= DATA_LEN){
            	printf("\nLT Remainder %d",_remainderTemp);
            }
            else{
            	printf("\nGT Remainder %d",_remainderTemp);
            }*/
...
/*
 * Degiskenler local olacak böylece task bağımsız olacak
 * Video Kaydı taskı
 */
